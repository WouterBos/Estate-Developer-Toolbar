...
function publish(symbolSet) {
	...
	IO.mkPath((publish.conf.outDir+publish.conf.symbolsDir).split("/"));
	...
	IO.saveFile(publish.conf.outDir, (JSDOC.opt.D.index=="files"?"classes":"index")+publish.conf.ext, classesIndex);
	...
	// copy static files
	copyFiles(publish.conf.templatesDir+publish.conf.staticDir, publish.conf.outDir, 32);
}

/** Copy files from src to dst, and possibly recurse. */
function copyFiles(src, dst, recurse) {
   IO.ls(src, recurse).reverse().forEach(function($){
      var d = $.substr(src.length).match(/^.*\/||/);
      try {
	 IO.mkPath((dst+d).split("/"));
	 IO.copyFile($, dst+d);
      } catch(e) { print(e.message); quit(); }
   });
}
